...
| Field                | Value                                                                                     |
| -------------------- | ----------------------------------------------------------------------------------------- |
| xurl-rs commit       | `b0779c1`, the `v3.2.0` tag (the released binary; Homebrew `xurl-rs` 3.2.0 bottle)        |
| Binary self-report   | `xr 3.2.0`                                                                                |
| Contract documented  | 3.2.0: bare `auth status` array, `next_step` and the reason catalog, 27 shortcuts         |
| Harness result       | `tests/contract.sh`: 141 checks passing against `target/debug/xr` from that tag           |

`main` documents the released binary; `dev` documents the upstream `dev` head, so the next bundle release ships
beside the next `xr` release with the tree already written. When any row above moves, re-run the harness and update
the row in the same PR. Consumers install from the head of `main` with the bundle's own `VERSION`; nothing pins the
bundle to a binary version on either side.
...
1. Build the target: `cd ~/dev/xurl-rs && git checkout <tag or dev> && cargo build`. Note the commit.
