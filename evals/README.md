...
| [eval-02-search-and-process.md](eval-02-search-and-process.md)                         | Discovery; `scripts/paginate.sh` selection; cursor-paginated read                                          | No        | F1 F2 F4 F5 F11  | `paginate.sh`, `agent-flags.md`, `search-and-process.md` change                                      |
...
| F3  | `references/output-envelope.md` § "`next_step`" and § "Exit 77 recipe"                                              | `next_step.action` is `register-app` / `sign-in` / `select-app` / `inspect-store` at exit 77, `enroll-app` on a 403 `forbidden`, `show-help` on `unknown-command`; an unrecognized action is a default branch that shows the step, never one that runs it; `command` is verbatim-safe, `template` needs user values; `rate-limited` carries none |
