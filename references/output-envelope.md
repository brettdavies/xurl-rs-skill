...
Three exceptions, all verified on the 3.2.0 release:
...
- `auth status` and `auth apps list` print a bare top-level array with no `status` key (see below).
...
`mentions`, `like` … `unbookmark`, `bookmarks`, `likes`, `follow` … `followers`, `mute` / `unmute`, `dm`, `dms`,
`usage`, `usage credits`, `media upload`, `media status`, and raw mode) prints the response body as the API sent it:
...
```json
{ "status": "ok", "message": "App \"my-app\" registered." }
```

Verbs that never touch the API answer `status: "ok"` with their own keys beside it, with two exceptions listed first:

| Verb                                                                                            | Top-level keys beside `status`                                             |
| ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `auth status`, `auth apps list`                                                                 | none: a bare top-level array, no `status` (see below)                      |
...
| `skill install --all`                                                                           | `action`, `installations`, `exit_code`                                     |
| `skill update --all`                                                                            | one per-host record per known host, printed as separate documents          |
...
**`auth status` and `auth apps list` print a bare array**: the shape is `[{...}, ...]`, with no `status` key and no
wrapper object. Every jq path into it starts at `.[]`:

```bash
xr auth status --output json | jaq -r '.[].name'
xr auth status --output json | jaq -r '.[] | select(.default) | .name'
xr auth apps list --output json | jaq -c '.[] | {name, client_id_hint, oauth2_users, bearer}'
```

An empty store answers `[]`, so a loop over `.[]` needs no zero-app special case. A development build ahead of 3.2.0
wraps the same entries as `{"status":"ok","apps":[...]}`; `(.apps? // .)[]` reads both shapes.
...
- It echoes arguments as given: `xr reply <status URL> … --dry-run` reports the URL as `post_id`, and `xr mute @handle
...
- It runs **after** the verb's own confirmation gate: `delete`, `auth clear`, and `auth apps remove` answer `reason:
  ...
  and the live call both. No other write verb takes `--force`; `xr mute @x --force` is `invalid-args`.
...
`skill update --all` prints one document per known host and refreshes every host, cloning into the ones that had no
installation; pass a single `<host>` to touch only that one.
...
| `exit_code` | `reason`(s)                                                                                                                                                                                            |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0           | success (an API document, `status: ok`, the bare `auth status` array, or a successful dry-run)                                                                                                         |
...
`bookmark`, `mute`; anything else answers `unknown-schema` with the list in `known_schemas`.
