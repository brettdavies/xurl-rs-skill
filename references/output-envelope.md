...
- It echoes arguments as given: `xr reply <status URL> … --dry-run` reports the URL as `post_id`; the live request
  carries the extracted id.
- It runs **after** the verb's own confirmation gate: `delete`, `auth clear`, and `auth apps remove` answer `reason:
  ...
  and the live call both.

Emitted by every write op when `--dry-run` is set. Read ops ignore `--dry-run` and run for real.
...
| `confirmation-required`                                                             | 1                       | A destructive verb ran without a TTY or `--no-interactive` and no `--force` | Re-run with `--force` after the user confirms (also needed under `--dry-run`)                               |
