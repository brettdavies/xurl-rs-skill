...
for. It is intentionally a **drift-resistant pointer** rather than a copy of the X docs, because the API changes faster
than this bundle is republished.
...
to plan an integration, read the per-endpoint doc first, because context and scope requirements vary even within a
single category.
...
Tier categories historically: Free, Basic, Pro, Enterprise. The **specific names, caps, and prices change**. Verify
...
`xr` ships 34 shortcut verbs over the common endpoints (posts, lookups, search, timelines, likes, reposts, bookmarks,
follows, mutes and blocks with their list reads, DMs, broadcast chat moderators, usage, media). For everything else,
drop to raw mode and consult the docs category index:
...
| Broadcasts       | Not indexed in `x-api/llms.txt` on the verified date; see below     |
