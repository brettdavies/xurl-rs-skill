...
#     envelope_reason FILE JQ       Prints the `reason` of an error envelope stored
#                                   in FILE, or nothing when FILE is not one.
...
# xr writes error envelopes to stderr. Callers capture stderr to a file and
# ask here whether it holds `{"status":"error","reason":...}`; a non-JSON
# stderr (a clap usage error, a stray warning) yields an empty string.
envelope_reason() {
  local file=$1 jq_bin=$2 reason
  [ -s "$file" ] || return 0
  reason=$("$jq_bin" -r 'if .status == "error" then .reason // "" else "" end' <"$file" 2>/dev/null) || reason=""
  printf '%s' "$reason"
}
