...
check "auth status: bare array, empty" 0 out '[]' -- "$X" auth status --output json --raw
check "auth status: no status key" 0 out '!"status"' -- "$X" auth status --output json
check "auth status: no apps wrapper" 0 out '!"apps"' -- "$X" auth status --output json
check "auth apps list: bare array" 0 out '[]' -- "$X" auth apps list --output json --raw
...
check "mute --dry-run echoes the handle" 0 out '"target_username": "@noisy"' -- "$X" mute @noisy --dry-run --output json
check "mute --force: invalid-args" 2 err '"reason": "invalid-args"' -- "$X" mute @noisy --force --dry-run --output json
check "block: not a 3.2.0 verb" 2 err '"reason": "invalid-args"' -- "$X" block @spammer --output json
check "blocked: not a 3.2.0 verb" 2 err '"reason": "unknown-command"' -- "$X" blocked --output json
...
check "followers ignores --dry-run" 77 err '"reason": "auth-required"' -- "$X" followers --dry-run --output json
...
check "skill install --all --dry-run: installations" 0 out '"installations"' -- "$X" skill install --all --dry-run --output json
check "skill update --all --dry-run: per-host documents" 0 out '"host": "opencode"' -- "$X" skill update --all --dry-run --output json
check "skill update --all --dry-run: no aggregate" 0 out '!"installations"' -- "$X" skill update --all --dry-run --output json
check "skill update --all --dry-run: six documents" 0 out '6' -- bash -c "'$X' skill update --all --dry-run --output json | jaq -c . | wc -l"
check "schema --list: message rows" 0 out '"message"' -- "$X" schema --list --output json
check "schema --list: mute" 0 out 'mute               ApiResponse<MutingResult>' -- "$X" schema --list
check "schema --list: no block" 0 out '!block ' -- "$X" schema --list
check "schema mute: positional" 0 out 'muting' -- "$X" schema mute --output json
...
check "validate --schema mute" 0 out '"valid": true' -- bash -c "printf '%s' '{\"data\":{\"muting\":true}}' | '$X' validate --schema mute --output json"
check "validate unknown schema" 1 err '"reason": "unknown-schema"' -- bash -c "printf '{}' | '$X' validate --schema tweet --output json"
check "validate known_schemas: 15 names" 0 out '15' -- bash -c "printf '{}' | '$X' validate --schema tweet --output json 2>&1 | jaq '.known_schemas | length'"
...
check "examples: mute line" 0 out 'xr mute @noisy --output json' -- "$X" examples
check "examples: usage credits" 0 out 'xr usage credits --output json' -- "$X" examples
check "--help: mute command" 0 out 'mute         Mute a user' -- "$X" --help
check "--help: no blocked command" 0 out '!blocked' -- "$X" --help
check "--help: EXIT CODES section" 0 out 'EXIT CODES:' -- "$X" --help
check "mute --help: USERNAME positional" 0 out '<USERNAME>' -- "$X" mute --help
check "usage --help: credits subcommand" 0 out 'credits' -- "$X" usage --help
check "schema count is 35" 0 out '35' -- bash -c "'$X' schema --list | wc -l"
...
check "followers --cursor -n 5: token + size" 0 log 'followers?max_results=5&user.fields=created_at%2Cdescription%2Cpublic_metrics%2Cverified&pagination_token=abc' -- "$X" followers --cursor abc -n 5 --output json
check "following --after: token" 0 log 'pagination_token=abc' -- "$X" following --after abc --output json
check "followers resolves /2/users/me first" 0 log '/2/users/me' -- "$X" followers --output json
check "followers -n 500 clamps to 100" 0 log 'max_results=100' -- "$X" followers -n 500 --output json
...
check "mute: lookup then POST /muting" 0 log '"path": "/2/users/42/muting"' -- "$X" mute @noisy --output json
check "unmute: DELETE /muting/<target>" 0 log '/2/users/42/muting/7' -- "$X" unmute noisy --output json
...
check "paginate.sh: followers" 0 out '"username":"u"' -- "$ROOT/scripts/paginate.sh" --max-pages 1 -- "$X" followers -n 5
...
check "dry-run-gate.sh: mute goes live" 0 log '"path": "/2/users/42/muting"' -- "$ROOT/scripts/dry-run-gate.sh" --yes -- "$X" mute @noisy
